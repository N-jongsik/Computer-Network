...
struct pkt packets[WINDOW_SIZE];
bool packet_sent[WINDOW_SIZE] = { false };
int base = 0;
int nextseqnum = 0;
...
{   if (nextseqnum < base + WINDOW_SIZE) {
    ...
    packet_sent[nextseqnum % WINDOW_SIZE] = true;

    // Save the packet in the window
    packets[nextseqnum % WINDOW_SIZE] = packet;
    nextseqnum++;
}
else {
    // If the window is full, the message is ignored
        printf("Window is full, ignoring message.\n");
    }
}
...
struct pkt packet;
{
    ...
    if (Checksum(packet) == packet.checksum) {
        // Check if the packet is an ACK for one or more packets
        if (packet.acknum >= base && packet.acknum < nextseqnum) {
            // Update sender base number
            int acked_packet = packet.acknum - base;
            packet_sent[acked_packet % WINDOW_SIZE] = false;
            while (!packet_sent[base % WINDOW_SIZE]) {
                base++;
            }

            // Stop timer if all packets have been ACKed
            if (base == nextseqnum) {
                stoptimer(A,packet.seqnum);
            }
            else {
...
A_timerinterrupt(int seqnum)
{
    ...
    if (packet_sent[index]) {
        if (TRACE > 2)
            printf("TIMER INTERRUPT: resending packet with sequence number %d\n", seqnum);
        tolayer3(A, packets[index]);
        //starttimer(A, TIMEOUT, seqnum);
    }
}
...
A_init()
{
    // Initialize the sequence number and base number
    nextseqnum = 0;
    base = 0;
    ...
    for (int i = 0; i < WINDOW_SIZE; i++) {
        packets[i].seqnum = i;
        packets[i].acknum = 0;
        memset(packets[i].payload, 0, sizeof(packets[i].payload));
        packets[i].checksum = calculate_checksum(packets[i]);
    }

    // Initialize the packet_sent array to false
    memset(packet_sent, false, sizeof(packet_sent));
}
